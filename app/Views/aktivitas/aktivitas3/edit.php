...
<head>
  ...
  <title>Edit Rekap BKK — FinanceOS</title>
...
function EditPage() {
  ...
  const [noRec, setNoRec] = useState(rekap.no_rec || '');
  const [tgl,   setTgl]   = useState(rekap.tgl    || '');
  const [idBkk, setIdBkk] = useState(String(rekap.id_bkk || ''));
  const [ket,   setKet]   = useState(rekap.ket    || '');
  const [errors, setErrors] = useState({});

  const selectedBkk = bkkList.find(b => String(b.id) === idBkk);
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6 fade-in">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl font-bold text-slate-800">Edit Rekap BKK</h1>
          <p className="text-sm text-slate-500 mt-0.5">Perbarui data rekapitulasi</p>
        </div>
      </div>
      ...
      <div className="flex items-center gap-2 px-4 py-2.5 bg-amber-50 border border-amber-200 rounded-xl text-amber-700 text-sm">
        ...
        Mengubah data rekap: <span className="font-semibold ml-1">{rekap.no_rec}</span>
      </div>
        ...
        <div className="flex items-center justify-between pt-2 border-t border-slate-100">
          ...
          <div className="flex items-center gap-3">
            ...
            <button type="submit"
              ...
              <Icon name="Save" size={15}/>Simpan Perubahan
            </button>
          </div>
        </div>
      </form>
    ...
    <div className="min-h-screen bg-slate-100">
      ...
      <div className={`sidebar-transition ${collapsed ? 'ml-16':'ml-64'}`}>
        ...
        <main className="pt-16 min-h-screen">
          <div className="p-6"><EditPage/></div>
        </main>
      </div>
