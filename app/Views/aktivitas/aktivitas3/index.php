    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md fade-in">
        <div className="p-6">
          ...
          <h3 className="text-lg font-semibold text-slate-800">Hapus Rekap BKK?</h3>
          <p className="text-sm text-slate-500 mt-2">
            Data rekap <strong>{item.no_rec}</strong> akan dihapus permanen. Tindakan ini tidak dapat dibatalkan.
          </p>
        </div>
        <div className="flex gap-3 px-6 pb-6">
          ...
          <a href={`/46124026/aktivitas/aktivitas3/hapus_l1H/${item.id}`}
...
function RekapBKK() {
  ...
  const currentPath = '/46124026/aktivitas/aktivitas3';

  const stats = useMemo(() => ({
    total    : data.length,
    bulanIni : data.filter(r => r.tgl && r.tgl.startsWith(new Date().toISOString().slice(0,7))).length,
    bkkUnik  : new Set(data.map(r => r.id_bkk)).size,
  }), [data]);
  ...
  const filtered = useMemo(() => {
    ...
    return data.filter(r =>
      search === '' ||
      (r.no_rec && r.no_rec.toLowerCase().includes(q)) ||
      (r.no_bkk && r.no_bkk.toLowerCase().includes(q)) ||
      (r.ket    && r.ket.toLowerCase().includes(q))
    );
  }, [data, search]);
        ...
        <header className={`fixed top-0 right-0 z-20 flex items-center justify-between h-16 bg-white border-b border-slate-200 px-4 shadow-sm sidebar-transition ${collapsed ? 'left-16' : 'left-64'}`}>
          <div className="flex items-center gap-3">
            ...
            <div className="hidden sm:flex items-center gap-1.5 text-sm">
              ...
              <Icon name="ChevronRight" size={13} className="text-slate-400"/>
              <span className="font-semibold text-slate-800">Rekap BKK</span>
            </div>
          </div>
          <div className="flex items-center gap-2">
            ...
            <a href="/46124026/aktivitas/aktivitas3/tambah_l1H"
          ...
          <div className="p-6 space-y-6 fade-in">
            ...
            <div>
              <h1 className="text-xl font-bold text-slate-800 flex items-center gap-2">
                <Icon name="ClipboardCheck" size={20} className="text-brand-600"/> Rekap BKK
              </h1>
              <p className="text-sm text-slate-500 mt-0.5">Kelola rekapitulasi bukti kas keluar</p>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex items-start gap-4">
                ...
                <div>
                  <p className="text-xs font-semibold text-slate-500 uppercase tracking-wide">Total Rekap</p>
              ...
              <div className="bg-white p-4 rounded-xl border border-amber-100 shadow-sm flex items-start gap-4">
                ...
                <div>
                  <p className="text-xs font-semibold text-slate-500 uppercase tracking-wide">BKK Tercakup</p>
                  <p className="text-2xl font-bold text-amber-700 mt-0.5">{stats.bkkUnik} <span className="text-xs font-normal text-slate-400">dokumen</span></p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-xl border border-slate-200 shadow-sm">
              <div className="flex flex-col sm:flex-row sm:items-center gap-3 px-5 py-4 border-b border-slate-100">
                <div className="flex items-center gap-2 bg-slate-100 rounded-lg px-3 py-2 flex-1 max-w-sm">
                  ...
                  <input
                    type="text"
                    placeholder="Cari no. rekap, BKK, keterangan..."
                    value={search}
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="bg-slate-50 border-b border-slate-200">
                      ...
                      <th className="px-5 py-3 text-left text-xs font-semibold text-slate-500 uppercase tracking-wide">No. Rekap</th>
                    ...
                    {filtered.length === 0 ? (
                      <tr>
                        <td colSpan="6" className="py-14 text-center text-slate-500">
                          ...
                          <p className="font-medium">Tidak ada data rekap BKK ditemukan.</p>
                          <a href="/46124026/aktivitas/aktivitas3/tambah_l1H" className="mt-3 inline-flex items-center gap-2 px-4 py-2 text-sm font-medium text-white bg-brand-600 hover:bg-brand-700 rounded-lg transition-colors">
                      ...
                      <tr key={row.id} className="hover:bg-slate-50 transition-colors">
                        ...
                        <td className="px-5 py-3.5">
                          <code className="font-mono text-xs font-semibold text-slate-600 bg-slate-100 px-2.5 py-1 rounded">
                            {row.no_bkk || '-'}
                          </code>
                        </td>
                        <td className="px-5 py-3.5 text-slate-500 max-w-xs truncate">{row.ket || '-'}</td>
                        <td className="px-5 py-3.5">
                          <div className="flex items-center justify-center gap-1">
                            <a href={`/46124026/aktivitas/aktivitas3/lihat_l1H/${row.id}`} title="Detail"
                              className="p-1.5 rounded-lg text-slate-400 hover:text-blue-600 hover:bg-blue-50 transition-colors">
                              <Icon name="Eye" size={15}/>
                            </a>
                            <a href={`/46124026/aktivitas/aktivitas3/edit_l1H/${row.id}`} title="Edit"
              ...
              {filtered.length > 0 && (
                <div className="px-5 py-3 border-t border-slate-100 flex items-center justify-between text-xs text-slate-400">
                  <span>Menampilkan {filtered.length} rekap BKK</span>
...
ReactDOM.createRoot(document.getElementById('root')).render(<RekapBKK/>);
</script>
</body>
</html>
