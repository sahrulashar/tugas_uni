...
function EditPage() {
  ...
  return (
    <div className="min-h-screen bg-slate-100">
      <Sidebar collapsed={collapsed} currentPath="/46124026/aktivitas/aktivitas1"/>
      ...
      <div className={`sidebar-transition ${collapsed ? 'ml-16' : 'ml-64'}`}>
        <header className={`fixed top-0 right-0 z-20 flex items-center justify-between h-16 bg-white border-b border-slate-200 px-4 shadow-sm sidebar-transition ${collapsed ? 'left-16' : 'left-64'}`}>
          <div className="flex items-center gap-3">
            ...
            <div className="hidden sm:flex items-center gap-1.5 text-sm">
              ...
              <a href="/46124026/aktivitas/aktivitas1" className="text-slate-500 hover:text-brand-600">Rencana Beli</a>
              <Icon name="ChevronRight" size={13} className="text-slate-400"/>
        ...
        <main className="pt-16 min-h-screen">
          <div className="p-6 max-w-4xl mx-auto space-y-6 fade-in">
            ...
            <div className="flex items-center gap-3">
              <a href="/46124026/aktivitas/aktivitas1" className="p-2 bg-white rounded-lg border border-slate-200 hover:bg-slate-50 shadow-sm transition-colors">
                <Icon name="ArrowLeft" size={16}/>
              </a>
              <div>
                <h1 className="text-xl font-bold text-slate-800">Edit Rencana Beli</h1>
                <p className="text-sm text-slate-500">
                  Perbarui data dokumen <code className="font-mono text-brand-600 bg-brand-50 px-1.5 py-0.5 rounded text-xs">{rbeli.no_rbeli}</code>
                </p>
              </div>
            </div>
            ...
            <form action="/46124026/aktivitas/aktivitas1/update_l1H" method="POST">
                ...
                <div className="p-6 space-y-5">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    <div>
                      ...
                      <input type="text" name="no_rbeli" required defaultValue={rbeli.no_rbeli}
                    ...
                    <div>
                      ...
                      <input type="date" name="tgl" required defaultValue={rbeli.tgl}
                    ...
                    <select name="id_supp" required
                      ...
                      {supplier.map(s => (
                        <option key={s.id_supplier} value={s.id_supplier}
                          selected={String(s.id_supplier) === String(rbeli.id_supp)}>
                          {s.nama_supplier}
                        </option>
                      ))}
                    </select>
                  ...
                  <div>
                    ...
                    <textarea name="kete" rows="2"
                      className="w-full px-4 py-2.5 text-sm border border-slate-300 rounded-xl focus:ring-2 focus:ring-brand-500 focus:border-brand-500 outline-none resize-none">
                      {rbeli.kete}
                    </textarea>
                  </div>
                </div>
              ...
              <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden mb-5">
                <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
                  ...
                  <button type="button" onClick={addRow}
                    className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-brand-700 bg-brand-50 hover:bg-brand-100 rounded-lg transition-colors">
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="bg-slate-50 border-b border-slate-200">
                        ...
                        <th className="px-4 py-3 text-left text-xs font-semibold text-slate-500 uppercase tracking-wide">No. Faktur</th>
                        <th className="px-4 py-3 text-left text-xs font-semibold text-slate-500 uppercase tracking-wide">Nilai (Rp)</th>
              ...
              <div className="flex items-center justify-end gap-3">
                <a href="/46124026/aktivitas/aktivitas1"
...
ReactDOM.createRoot(document.getElementById('root')).render(<EditPage/>);
</script>
</body>
</html>
