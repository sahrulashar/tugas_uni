...
<head>
  ...
  <title>Detail Supplier — FinanceOS</title>
...
function LihatSupplierPage() {
  const data = window.__SUPPLIER__ || {};
  const [collapsed, setCollapsed] = useState(false);
  const isAktif = data.status === 'Aktif';

  return (
    <div className="min-h-screen bg-slate-100">
      <Sidebar collapsed={collapsed} currentPath="/kas_keluar/supplier"/>
        ...
        <header className={`fixed top-0 right-0 z-20 flex items-center justify-between h-16 bg-white border-b border-slate-200 px-4 shadow-sm sidebar-transition ${collapsed ? 'left-16' : 'left-64'}`}>
          <div className="flex items-center gap-3">
            ...
            <div className="hidden sm:flex items-center gap-1.5 text-sm">
              ...
              <a href="/kas_keluar/supplier" className="text-slate-500 hover:text-brand-600">Supplier</a>
              <Icon name="ChevronRight" size={13} className="text-slate-400"/>
        ...
        <main className="pt-16 min-h-screen">
          <div className="p-6 max-w-4xl mx-auto space-y-5 fade-in">
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                <a href="/kas_keluar/supplier" className="p-2 bg-white rounded-lg border border-slate-200 hover:bg-slate-50 shadow-sm transition-colors">
                  <Icon name="ArrowLeft" size={16}/>
                </a>
                <div>
                  <h1 className="text-xl font-bold text-slate-800">Detail Supplier</h1>
                  <p className="text-sm text-slate-500">Informasi lengkap vendor & penyedia barang/jasa</p>
                </div>
              </div>
              <a href={`/kas_keluar/edit_supplier/${data.id_supplier}`} className="flex items-center gap-2 px-4 py-2 text-sm font-medium text-white bg-brand-600 hover:bg-brand-700 rounded-lg shadow-sm transition-colors">
            ...
            <div className="bg-gradient-to-br from-brand-600 to-brand-800 rounded-2xl p-6 flex flex-col sm:flex-row sm:items-center gap-5 text-white shadow-sm">
              ...
              <div className="flex-1">
                <div className="flex flex-wrap items-center gap-2 mb-2">
                  <code className="font-mono text-sm font-bold bg-white/20 px-3 py-1 rounded-lg">{data.kode_supplier}</code>
                ...
                <h2 className="text-2xl font-bold">{data.nama_supplier}</h2>
                {data.alamat && (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
              ...
              <div className="lg:col-span-2 bg-white rounded-xl border border-slate-200 shadow-sm">
                <div className="px-6 py-4 border-b border-slate-100 flex items-center gap-2">
                  ...
                  <h3 className="font-semibold text-slate-800 text-sm">Informasi Supplier</h3>
                </div>
                <div className="px-6">
                  <DetailRow label="ID Supplier">
                    <span className="font-mono text-xs text-slate-500 bg-slate-100 px-2 py-1 rounded">#{data.id_supplier}</span>
                  </DetailRow>
                  <DetailRow label="Kode Supplier">
                    <code className="font-mono font-semibold text-brand-600 bg-brand-50 px-3 py-1 rounded-lg text-sm border border-brand-100">{data.kode_supplier}</code>
                  </DetailRow>
                  <DetailRow label="Nama Supplier">
                    <span className="font-semibold text-slate-800">{data.nama_supplier}</span>
                  </DetailRow>
                  <DetailRow label="Alamat">
                    <span className="text-slate-700">{data.alamat || <span className="text-slate-400 italic">Tidak ada alamat tercatat</span>}</span>
                  </DetailRow>
              ...
              <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden h-fit">
                ...
                <div className="p-3 space-y-2">
                  <a href={`/kas_keluar/edit_supplier/${data.id_supplier}`} className="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium text-slate-700 hover:bg-amber-50 hover:text-amber-700 transition-colors group">
                  ...
                  {isAktif ? (
                    <a href={`/kas_keluar/hapus_supplier/${data.id_supplier}`} onClick={e => !confirm('Yakin ingin menonaktifkan supplier ini?') && e.preventDefault()} className="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium text-slate-700 hover:bg-red-50 hover:text-red-700 transition-colors group">
                      <span className="p-1 rounded-lg bg-red-50 text-red-500 group-hover:bg-red-100"><Icon name="PowerOff" size={14}/></span> Nonaktifkan
                    </a>
                  ) : (
                    <a href={`/kas_keluar/aktifkan_supplier/${data.id_supplier}`} className="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium text-slate-700 hover:bg-green-50 hover:text-green-700 transition-colors group">
                  ...
                  <a href="/kas_keluar/supplier" className="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium text-slate-700 hover:bg-slate-100 transition-colors group">
...
ReactDOM.createRoot(document.getElementById('root')).render(<LihatSupplierPage />);
</script>
</body>
</html>
