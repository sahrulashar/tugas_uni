...
function StatCard({ tipe, total, aktif, tidakAktif }) {
  const colors = {
    'Aset':        { bg:'bg-blue-50',   border:'border-blue-100',   icon:'text-blue-600',   iconName:'Landmark' },
    'Liabilitas':  { bg:'bg-red-50',    border:'border-red-100',    icon:'text-red-600',    iconName:'CreditCard' },
    'Ekuitas':     { bg:'bg-purple-50', border:'border-purple-100', icon:'text-purple-600', iconName:'PiggyBank' },
    'Pendapatan':  { bg:'bg-green-50',  border:'border-green-100',  icon:'text-green-600',  iconName:'TrendingUp' },
    'Beban':       { bg:'bg-orange-50', border:'border-orange-100', icon:'text-orange-600', iconName:'TrendingDown' },
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md fade-in">
        ...
        <div className="flex gap-3 px-6 pb-6">
          ...
          <a href={isAktif ? `/kas_keluar/hapus_coa/${coa.id_coa}` : `/kas_keluar/aktifkan_coa/${coa.id_coa}`}
      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="bg-slate-50 border-b border-slate-200">
            ...
            <th className="px-5 py-3 text-left text-xs font-semibold text-slate-500 uppercase tracking-wide">Kode COA</th>
            <th className="px-5 py-3 text-left text-xs font-semibold text-slate-500 uppercase tracking-wide">Nama Akun</th>
            <th className="px-5 py-3 text-left text-xs font-semibold text-slate-500 uppercase tracking-wide">Tipe Akun</th>
            <th className="px-5 py-3 text-left text-xs font-semibold text-slate-500 uppercase tracking-wide">Saldo Normal</th>
        ...
        <tbody className="divide-y divide-slate-100">
          {data.map((row, i) => (
            <tr key={row.id_coa} className={`hover:bg-slate-50 transition-colors ${row.status !== 'Aktif' ? 'opacity-60' : ''}`}>
              ...
              <td className="px-5 py-3.5 font-medium text-slate-800">{row.nama_coa}</td>
              <td className="px-5 py-3.5"><TipeBadge tipe={row.tipe_akun}/></td>
              ...
              <td className="px-5 py-3.5">
                <div className="flex items-center justify-center gap-1">
                  {/* Lihat */}
                  <a href={`/kas_keluar/lihat_coa/${row.id_coa}`}
                    title="Detail"
                    className="p-1.5 rounded-lg text-slate-400 hover:text-blue-600 hover:bg-blue-50 transition-colors">
                    <Icon name="Eye" size={15}/>
                  </a>
                  {/* Edit */}
                  <a href={`/kas_keluar/edit_coa/${row.id_coa}`}
                    title="Edit"
...
function CoaPage() {
  ...
  const tipeList = useMemo(() => {
    const order = ['Aset','Liabilitas','Ekuitas','Pendapatan','Beban'];
    const found = [...new Set(rawCoa.map(r => r.tipe_akun))];
    return ['Semua', ...order.filter(t => found.includes(t))];
  }, [rawCoa]);
  ...
  const filtered = useMemo(() => {
    return rawCoa.filter(row => {
      ...
      const matchTipe   = filterTipe   === 'Semua' || row.tipe_akun === filterTipe;
      const matchStatus = filterStatus === 'Semua' || row.status === filterStatus;
      return matchSearch && matchTipe && matchStatus;
    });
  }, [rawCoa, search, filterTipe, filterStatus]);

  /* Build stats from rawStats (getTotalPerTipe) or derive from rawCoa */
  const stats = useMemo(() => {
    if (rawStats.length > 0) return rawStats;
    const map = {};
    rawCoa.forEach(r => {
      if (!map[r.tipe_akun]) map[r.tipe_akun] = { tipe_akun: r.tipe_akun, total:0, aktif:0, tidak_aktif:0 };
      map[r.tipe_akun].total++;
      if (r.status === 'Aktif') map[r.tipe_akun].aktif++;
      else map[r.tipe_akun].tidak_aktif++;
    });
                  ...
                  <button
                    ...
                    {t !== 'Semua' && (
                      <span className={`ml-1.5 text-[10px] px-1.5 py-0.5 rounded-full font-semibold
                        ...
                        {rawCoa.filter(r => r.tipe_akun === t).length}
                      </span>
                    )}
                  </button>
                ))}
