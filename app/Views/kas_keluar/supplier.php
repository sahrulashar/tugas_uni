  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md fade-in">
        <div className="p-6">
          <div className={`w-12 h-12 rounded-full flex items-center justify-center mb-4 ${isAktif ? 'bg-red-100' : 'bg-green-100'}`}>
            <Icon name={isAktif ? 'AlertTriangle' : 'CheckCircle'} size={22} className={isAktif ? 'text-red-600' : 'text-green-600'}/>
          </div>
          <h3 className="text-lg font-semibold text-slate-800">
            {isAktif ? 'Nonaktifkan Supplier?' : 'Aktifkan Supplier?'}
          </h3>
          <p className="text-sm text-slate-500 mt-2">
            {isAktif
              ? <>Supplier <strong>{item.nama_supplier} ({item.kode_supplier})</strong> akan dinonaktifkan.</>
              : <>Supplier <strong>{item.nama_supplier} ({item.kode_supplier})</strong> akan diaktifkan kembali.</>
            }
          </p>
        </div>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md fade-in">
        <div className="p-6">
          ...
          <p className="text-sm text-slate-500 mt-2">
            Data supplier <strong>{item.nama_supplier} ({item.kode_supplier})</strong> akan dihapus permanen dari basis data. Tindakan ini tidak dapat dibatalkan.
          </p>
        </div>
        <div className="flex gap-3 px-6 pb-6">
          <button onClick={onCancel} className="flex-1 px-4 py-2.5 text-sm font-medium text-slate-700 bg-slate-100 hover:bg-slate-200 rounded-lg transition-colors">
...
function SupplierPage() {
  const rawSupplier = window.__SUPPLIER__ || [];
  ...
  const stats = useMemo(() => {
    ...
    const aktif = rawSupplier.filter(s => s.status === 'Aktif').length;
    const nonaktif = total - aktif;
    return { total, aktif, nonaktif };
  }, [rawSupplier]);

  const filtered = useMemo(() => {
    return rawSupplier.filter(row => {
      const q = search.toLowerCase();
      const matchSearch = search === '' ||
        (row.kode_supplier && row.kode_supplier.toLowerCase().includes(q)) ||
        (row.nama_supplier && row.nama_supplier.toLowerCase().includes(q)) ||
        (row.alamat && row.alamat.toLowerCase().includes(q));
      const matchStatus = filterStatus === 'Semua' || row.status === filterStatus;
      return matchSearch && matchStatus;
    });
  }, [rawSupplier, search, filterStatus]);
      ...
      <div className={`sidebar-transition ${collapsed ? 'ml-16' : 'ml-64'}`}>
        <header className={`fixed top-0 right-0 z-20 flex items-center justify-between h-16 bg-white border-b border-slate-200 px-4 shadow-sm sidebar-transition ${collapsed ? 'left-16' : 'left-64'}`}>
          ...
          <div className="flex items-center gap-2">
            ...
            <a href="/kas_keluar/tambah_supplier" className="flex items-center gap-2 px-4 py-2 text-sm font-medium text-white bg-brand-600 hover:bg-brand-700 rounded-lg shadow-sm transition-colors">
          ...
          <div className="p-6 space-y-6 fade-in">
            <div>
              <h1 className="text-xl font-bold text-slate-800 flex items-center gap-2">
                <Icon name="Truck" size={20} className="text-brand-600"/> Data Supplier
              </h1>
              <p className="text-sm text-slate-500 mt-0.5">Kelola daftar vendor dan pihak penyedia barang/jasa</p>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex items-start gap-4">
                ...
                <div>
                  <p className="text-xs font-semibold text-slate-500 uppercase tracking-wide">Total Supplier</p>
                  <p className="text-2xl font-bold text-slate-800 mt-0.5">{stats.total} <span className="text-xs font-normal text-slate-400">vendor</span></p>
                </div>
              </div>
              ...
              <div className="bg-white p-4 rounded-xl border border-green-100 shadow-sm flex items-start gap-4">
                ...
                <div>
                  <p className="text-xs font-semibold text-slate-500 uppercase tracking-wide">Supplier Aktif</p>
                  <p className="text-2xl font-bold text-green-700 mt-0.5">{stats.aktif} <span className="text-xs font-normal text-slate-400">vendor</span></p>
                </div>
              </div>
              ...
              <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex items-start gap-4">
                ...
                <div>
                  <p className="text-xs font-semibold text-slate-500 uppercase tracking-wide">Supplier Nonaktif</p>
                  <p className="text-2xl font-bold text-slate-700 mt-0.5">{stats.nonaktif} <span className="text-xs font-normal text-slate-400">vendor</span></p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-xl border border-slate-200 shadow-sm">
              <div className="flex flex-col sm:flex-row sm:items-center gap-3 px-5 py-4 border-b border-slate-100">
                <div className="flex items-center gap-2 bg-slate-100 rounded-lg px-3 py-2 flex-1 max-w-sm">
                  ...
                  <input
                    type="text"
                    placeholder="Cari kode, nama supplier, atau alamat..."
                    value={search}
                ...
                <div className="text-xs text-slate-400 ml-auto whitespace-nowrap">
                  <span className="font-semibold text-slate-700">{filtered.length}</span> dari {rawSupplier.length} supplier
                </div>
              </div>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="bg-slate-50 border-b border-slate-200">
                      ...
                      <th className="px-5 py-3 text-left text-xs font-semibold text-slate-500 uppercase tracking-wide">Nama Supplier</th>
                      <th className="px-5 py-3 text-left text-xs font-semibold text-slate-500 uppercase tracking-wide">Alamat</th>
                    ...
                    {filtered.length === 0 ? (
                      <tr>
                        <td colSpan="6" className="py-14 text-center text-slate-500">
                          ...
                          <p className="font-medium">Tidak ada data supplier ditemukan.</p>
                        </td>
                      </tr>
                    ) : filtered.map((row, i) => (
                      <tr key={row.id_supplier} className={`hover:bg-slate-50 transition-colors ${row.status !== 'Aktif' ? 'opacity-60' : ''}`}>
                        ...
                        <td className="px-5 py-3.5 text-slate-600 max-w-xs truncate">{row.alamat || '-'}</td>
                        <td className="px-5 py-3.5">
